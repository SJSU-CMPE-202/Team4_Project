...
# The purpose of this program is to parse a TLV (Tag, Length, Value) file
# that contains certificates and other relevant information into readable
# information.
...
def parse_read(file_name):
    ...
    for key, value in SCF_header.items():
        print(key, ":  ", value)
    input("Press enter to continue")
    while position < len(bytes_pages) - 1:
        SCF_body, position = parse_body(bytes_pages, position)
        print("Byte position: ",position)
        for key, value in SCF_body.items():
            print(key, ":  ", value)
